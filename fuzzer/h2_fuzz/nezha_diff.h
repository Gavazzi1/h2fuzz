...
uint64_t *cov_buff_idx = nullptr;

// Array holding per-lib bitcounts.
int *bitcounts = nullptr;
...
struct ValContainerCallback {
    CallbackRet *vals;
    int size;
} vcont_callback = { nullptr, 0 };


extern "C" ValContainerCallback *LLVMFuzzerNezhaOutputs() {
    if (!ret_vals)
        return nullptr;
    vcont_callback.vals = ret_vals;
    vcont_callback.size = total_libs;
    return &vcont_callback;
}
...
void diff_init() {
    num_bitcounters = 1;//__sanitizer_get_number_of_counters();

    ret_vals = (CallbackRet *) calloc(total_libs, sizeof(CallbackRet));
    ...
    assert(bitset_cur != nullptr && "error allocating bitset_cur*");
    for (int i = 0; i < total_libs; i++) {
        bitset_cur[i] = (uint8_t *) calloc(num_bitcounters, sizeof(uint8_t));
        assert(bitset_cur[i] != nullptr && "error allocating bitset_cur");
    }

    bitset_old = (uint8_t **) calloc(total_libs, sizeof(uint8_t *));
    assert(bitset_old != nullptr && "error allocating bitset_old*");
    for (int i = 0; i < total_libs; i++) {
        bitset_old[i] = (uint8_t *) calloc(num_bitcounters, sizeof(uint8_t));
        assert(bitset_old[i] != nullptr && "error allocating bitset_old");
    }

    assert(ret_vals != nullptr && "error allocating ret_vals");
    assert(cov_buff_idx != nullptr && "error allocating cov_buff_idx");
    assert(bitcounts != nullptr && "error allocating bitcounts");
    assert(ecnt != nullptr && "error allocating ecnt");
}
